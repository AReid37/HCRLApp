...
namespace HCResourceLibraryApp.Layout
{
    public static class LibrarySearch
    {
        ...
        // RUDIMENTARY LIBRARY fields
        static bool rud_searchAlpha = false;
        const string rud_findKey = "find:";
        static string rud_findPhrase = "";
            ...
            if (libraryIsSetup)
            {
                ...
                if (rud_findPhrase.IsNotNEW())
                {
                    foreach (ResContents resCon in _resLibrary.Contents)
                    {
                        ...
                        if (megaRCString.ToLower().Contains(rud_findPhrase.ToLower()))
                ...
                else FormatLine($"{Ind14}No results matched '{rud_findPhrase}' ... ");
                HorizontalRule(subMenuUnderline);
                ...
                FormatLine($"Precede text with '{rud_findKey}' to act as a search filter (case-insensitive).", ForECol.Accent);
                ...
                if (StyledInput("###").IsNotNE())
                {
                    ...
                    {
                        if (LastInput.StartsWith(rud_findKey))
                        {
                            if (LastInput.Substring(rud_findKey.Length).IsNotNEW())
                            {
                                rud_findPhrase = LastInput.Substring(rud_findKey.Length);
                                Format($"{Ind24}Searching for any contents containing '{rud_findPhrase}'...", ForECol.Correction);
                            }
                            else
                            {
                                Format($"{Ind24}Search filter '{rud_findPhrase}' cleared...", ForECol.Correction);
                                rud_findPhrase = "";
                            }
                        }
                        else
