        ...
        **/
        ...
        public bool DecodeLogInfo(string[] logData, VerNum earliestLibVersion, VerNum latestLibVersion)
        {
            ...
            if (logData.HasElements())
            {
                ...
                for (int llx = 0; isLegacyQ ^ isVer2Q == true; llx++)
                {
                        ...
                        if (!omitThisLine && !invalidLine && !withinOmitBlock)
                        {
                            ...
                            if (!withinASectionQ)
                            {
                                ...
                                else if (ContainsKeywordQ(out bool isSectionKeywordQ))
                                {
                                    ...
                                    {
                                        diMain.NoteIssue(ldx, "Log data line is not within a log section");
                            ...
                            if (withinASectionQ)
                            {
                                        ...
                                        if (logDataLine.Contains(kwAs))
                                        {
                                            ...
                                            if (addedParts.Length == 2)
                                            {
                                                ...
                                                if (added_contName.IsNotNEW())
                                                {
                                                    ...
                                                    if (!int.TryParse(added_contName, out _))
                                                    {
                                                        addedContentName = CharacterKeycodeSubstitution(FixContentName(ReplacePlaceholdersWithSubs(added_contName)));
                                                        Dbg.Log(ldx, $"Got final content name :: {addedContentName}");
                                                    }
                                                    else
                                                    {
                                                        decodeInfo.NoteIssue(ldx, "Internal name cannot be plain numbers");
                                                        Dbg.Log(ldx, "Added content name cannot be plain numbers; ");
                                                    }
                                                }
                                                else
                                                {
                                ...
                                if (currentSectionName == secAdt)
                                {
                                    ...
                                    {
                                        // setup
                                        string additDataID = null, additOptionalName = null, additRelatedName = null, additRelatedDataID = null;
                                        List<string> adtConDataIDs = new();
                                        ...
                                        if (logDataLine.Contains(kwBy))
                                        {
                                            ...
                                            if (additParts.Length == 2)
                                            {
                                                ...
                                                // part 1 - the content IDs
                                                if (remainder.IsNotNEW())
                                                {
                                                    /// tbd...
                                                    ParseDataIDs("...");
                                                    // also be weary of wordy data Ids, how are those handled?
                                                }
                                                else
                                                {
                                                    decodeInfo.NoteIssue(ldx, "Could not get content's data IDs");
                                                    Dbg.LogPart(ldx, "Could not get content's data IDs; ");
                                                }
                                                // part 2 - the related data ID
                                                ...
                                                // compile additional content
                                                if (additDataID.IsNotNEW() && additRelatedDataID.IsNotNEW())
                                                {
                                                    // tbd...
                                                }
                                                else
